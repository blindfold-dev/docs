...
All SDKs include **local mode** with 80+ regex-based entity types. No signup, no API key, no network calls — your data never leaves your infrastructure.
...
<Info>
  In local mode, no data leaves your infrastructure — everything runs in-process with zero network calls. Local mode supports 80+ regex-based entity types (emails, phones, credit cards, SSNs, IBANs, and more).
</Info>

Ready for NLP-powered detection (names, addresses, organizations), compliance policies, and audit logs? Continue below to set up the Cloud API.
