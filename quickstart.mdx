...
Blindfold automatically detects PII across multiple languages without any configuration. Native support for 9 languages plus zero-shot detection for 6+ more:
...
# Automatically detects entities in English and Spanish

# Works with many European languages
response = client.tokenize(
...
Define your own entity types for industry-specific data:

```python
# Detect custom entities using zero-shot learning
response = client.tokenize(
    "Order #ORD-2024-XYZ for customer ID CUST-456",
    entities=["order number", "customer identifier"]
)
# Detects custom entity types without any training
```

<Tip>
  See [Supported Entities](/essentials/supported-entities) for complete details on multilingual support and custom entity detection.
</Tip>
