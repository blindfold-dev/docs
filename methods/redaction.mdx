...
Redaction is a permanent privacy protection method that completely removes sensitive data from text by replacing it with generic placeholders (e.g., `<PERSON>`, `<EMAIL_ADDRESS>`). Unlike tokenization, redacted data cannot be restored.

**Example:**
```
Input:  "My name is John Doe and SSN is 123-45-6789"
Output: "My name is <PERSON> and SSN is <US_SSN>"
```

## How It Works

1. **Detection**: Blindfold identifies sensitive entities in your text
2. **Complete Replacement**: Each entity is replaced with a generic type label
3. **Permanent Removal**: Original values are discarded and cannot be recovered
4. **Type Preservation**: The type of data (email, SSN, etc.) is indicated but not the value
...
# "Customer <PERSON> (<EMAIL_ADDRESS>) reported an issue with order #12345"
```
...
# "User <EMAIL_ADDRESS> completed purchase"
```
...
redacted1 = client.redact("User: john@example.com")  # <EMAIL_ADDRESS>
redacted2 = client.redact("User: john@example.com")  # <EMAIL_ADDRESS>
...
<Tabs>
  ...
  <Tab title="cURL">
    ...
    # Response
    {
      "text": "Contact <PERSON> at <EMAIL_ADDRESS> or call <PHONE_NUMBER>",
      "entities_count": 3,
      "detected_entities": [
        {
          "entity_type": "PERSON",
...
# Output: "John Doe (SSN: <US_SSN>) paid with card <CREDIT_CARD>"
...
# Logs: "User <EMAIL_ADDRESS> failed to login from <IP_ADDRESS>"
```
...
<AccordionGroup>
  <Accordion title="Compliance Logs" icon="file-shield">
    ...
    # Logs: "<EMAIL_ADDRESS> performed password_reset"
    ```
  ...
  <Accordion title="Customer Feedback" icon="comment">
    ...
    # Stores: "Great service! Contact me at <EMAIL_ADDRESS>"
    ```
  ...
  <Accordion title="Public Dataset Creation" icon="database">
    ...
    # Public: ["<PERSON>, <EMAIL_ADDRESS>, <PHONE_NUMBER>", ...]
    ```
...
- **Type visibility**: Entity types are visible (e.g., `<EMAIL_ADDRESS>`)
- **Context preservation**: Surrounding text remains unchanged
- **Not encryption**: Redaction is removal, not encryption
