...
<Info>
  **Want to try without an API key?** All SDKs include local mode with 80+ regex-based entity types (emails, phones, credit cards, SSNs, and more). Just install the SDK and use `Blindfold()` with no arguments — no signup, no network calls, no data leaves your infrastructure. For NLP-powered detection (names, addresses, organizations) and compliance policies, continue with the Cloud API setup below.
</Info>
