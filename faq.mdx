...
<AccordionGroup>
  ...
  <Accordion title="How accurate is the PII detection?" icon="bullseye">
    ...
    **Tip**: Use `policy="strict"` for maximum detection or adjust `override_threshold` for your needs.
  </Accordion>
  ...
  <Accordion title="How do I handle false positives?" icon="filter">
    ...
    **1. Increase Detection Threshold**
    ...
    result = client.tokenize(
        text,
        policy="gdpr_eu",
        override_threshold=0.80  # Default is 0.35
    )
    ```
...
<AccordionGroup>
  <Accordion title="What's the API rate limit?" icon="gauge">
    ...
    **Handling rate limits:**
    ...
    Contact us for higher limits: hello@blindfold.dev
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="How is usage calculated?" icon="calculator">
    **1 API request = 1 call to any endpoint:**

    - `/tokenize` → 1 request
    - `/detokenize` → 1 request
    - `/mask` → 1 request
    - `/redact` → 1 request
    - Policy management → Free (no quota)

    **Text length doesn't matter:**
    - 10 words = 1 request
    - 10,000 words = 1 request

    **Batch processing:**
    - 3 separate calls = 3 requests
    - Consider batching similar text together
  </Accordion>
</AccordionGroup>
