...
<AccordionGroup>
  ...
  <Accordion title="Can I use Blindfold without an API key?" icon="wifi-slash">
    **Yes! All SDKs include local mode** with 80+ regex-based entity types, zero dependencies, and no API key required.

    In local mode, **no data ever leaves your infrastructure** — everything runs in-process with no network calls.
