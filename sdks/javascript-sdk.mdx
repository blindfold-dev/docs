...
Process PII in images — upload an image, and receive a modified PNG with PII protected. All image methods return an `ImageProcessResponse` containing the processed image and metadata.

### ImageProcessResponse

| Field | Type | Description |
|-------|------|-------------|
| `image` | `ArrayBuffer` | Processed PNG image |
| `detected_entities` | `DetectedEntity[]` | Entities found in the image |
| `entities_count` | `number` | Number of entities detected |
| `mapping` | `Record<string, string>` | Token-to-original mapping |
| `ocr_confidence` | `number \| null` | Average OCR confidence (0-100) |
...
const result = await client.imageDetect(readFileSync('screenshot.png'))
console.log(result.extracted_text)
console.log(result.ocr_confidence)
result.detected_entities.forEach(e => console.log(`${e.type}: ${e.text}`))
```
...
**Config type:** `ImageMaskConfig` — adds `chars_to_show` (default: 3), `from_end` (default: false), `masking_char` (default: `*`)
...
**Config type:** `ImageSynthesizeConfig` — adds `synthesis_language` (e.g., `en`, `de`, `fr`)
...
**Config type:** `ImageHashConfig` — adds `hash_type` (default: `sha256`), `hash_prefix` (default: `HASH_`), `hash_length` (default: 16)
...
**Config type:** `ImageEncryptConfig` — adds `encryption_key` (min 16 characters, uses tenant default if omitted)

### Common Config (ImageProcessConfig)

All image methods accept a config object with:

| Property | Type | Default | Description |
|----------|------|---------|-------------|
| `language` | `string` | `eng` | Tesseract OCR language code |
| `entities` | `string[]` | — | Entity types to detect |
| `score_threshold` | `number` | — | Minimum confidence (0.0-1.0) |
| `policy` | `string` | — | Detection policy name |

**Input:** All image methods accept `Blob | Buffer` as the first argument.
