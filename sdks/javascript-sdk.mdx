...
// Override policy settings for specific needs
const response2 = await client.tokenize(
  "Patient: John Doe, SSN: 123-45-6789",
  {
    policy: "hipaa_us",
    override_threshold: 0.30  // Lower threshold for testing
  }
);

// Override entities while keeping policy threshold
const response3 = await client.tokenize(
  "Custom data processing",
  {
    policy: "gdpr_eu",
    override_entities: ["person", "email address"]  // Only detect these
  }
);
```

<Info>
  Policies simplify your code and ensure consistent PII detection across your application. Create custom policies through the dashboard or API.
</Info>
...
- `config` (object, optional):
  ...
  - `override_threshold` (number): Override policy threshold
  - `override_entities` (string[]): Override policy entities
