...
All SDKs include **local mode** with 80+ regex-based entity types. No API key, no network calls — your data never leaves your infrastructure.
...
Local mode detects emails, phone numbers, credit cards, SSNs, IBANs, IP addresses, and 70+ more entity types using regex patterns. For NLP-powered detection (names, addresses, organizations), compliance policies, and audit logs, initialize with an API key to use the Cloud API.
