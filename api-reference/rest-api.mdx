...
<CardGroup cols={2}>
  ...
  <Card title="File Processing" icon="image" href="#file-processing-endpoints">
    Image PII detection and protection via OCR
  </Card>
...
These endpoints process PII in images. All accept `multipart/form-data` with a `file` field. The `/file/detect` endpoint returns JSON; all other `/file/` endpoints return **binary PNG** image data with metadata in response headers.
...
All `/file/` endpoints accept these multipart form fields:

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `file` | file | Yes | Image file (JPEG, PNG, TIFF, BMP, WebP) |
| `language` | string | No | Tesseract OCR language code (default: `eng`) |
| `entities` | string | No | Comma-separated entity types to detect |
| `score_threshold` | float | No | Minimum confidence (0.0-1.0) |
...
Detect PII in an image using OCR. Returns JSON (not an image).
...
Replace PII in an image with token labels. Returns a modified PNG.
...
No additional parameters beyond [common parameters](#common-parameters-1).

---

### POST /file/redact

Remove PII from an image with black boxes. Returns a modified PNG.
...
Partially hide PII in an image. Returns a modified PNG.
...
|-------|------|---------|-------------|
| `chars_to_show` | integer | `3` | Characters to keep visible |
| `from_end` | boolean | `false` | Show visible chars from end |
| `masking_char` | string | `*` | Character for masking |

---

### POST /file/synthesize

Replace PII in an image with realistic fake data. Returns a modified PNG.
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `synthesis_language` | string | `en` | Language for fake data: `en`, `cs`, `de`, `fr`, `es`, `it`, `pl`, `sk` |

---

### POST /file/hash

Replace PII in an image with deterministic hashes. Returns a modified PNG.
...
Encrypt PII in an image with AES encryption. Returns a modified PNG.
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `encryption_key` | string | — | Encryption key (min 16 chars). Uses tenant default if omitted. |
